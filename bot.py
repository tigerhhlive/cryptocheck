...
def detect_strong_candle(row, threshold=0.65):
...
def detect_atr_breakout(df, atr_col, multiplier=1.4):
...
def analyze_symbol(symbol, timeframe='15m'):
    ...
    confirmations = []
    if signal_type and 'bullish' in signal_type and rsi_val >= 50:
        confirmations.append("RSI")
    elif signal_type and 'bearish' in signal_type and rsi_val <= 50:
        confirmations.append("RSI")
...
def monitor():
    ...
    while True:
        ...
        if tehran_hour == 23 and tehran_min >= 59 and current_day != last_report_day:
            winrate = round((daily_hit_count / daily_signal_count) * 100, 1) if daily_signal_count > 0 else 0.0
